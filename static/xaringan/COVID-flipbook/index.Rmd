---
title: "A First Flipbook with flipbookr"
subtitle: "An Application to COVID Race to Open"
...
library(xaringan); library(flipbookr)
library(tidyverse); library(here); library(gganimate); library(magrittr)
...
```{r ORCOVID, eval=FALSE, message=FALSE, warning=FALSE, display_type="both", include=FALSE, results='hide'}
# The Race to Open
OR.County.COVID %>% filter(RankC > 26) %>%
    ...
    labs(title='Cases in Top 10 Oregon Counties', caption='Source: OHA Data', y="Confirmed COVID-19 Cases", subtitle='{frame_time}') +
...
animate(p1, nframes = 550, end_pause = 100, width = 1200, height = 900)
```

`r chunk_reveal("ORCOVID")`
