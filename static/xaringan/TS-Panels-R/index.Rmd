...
NYT.COVID %>% as_tibble() %>% left_join(., state_ranks %>% select(state, name)  %>% distinct(), by=c("state" = "name")) %>% as_tsibble(index=date, key=state) %>% autoplot(cases) + guides(color=FALSE) + labs(title="COVID-19 Cases in the 50 US States", y="New COVID-19 Cases") + facet_geo(~state.y, move_axes = FALSE) + scale_x_date(date_breaks = "6 months") + theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=1, size = 6))
```
...
NYT.COVID %>% 
  ...
  as_tsibble(index=date, key=state) %>% 
  model(ARIMA = ARIMA(New.Cases)) %>% 
  augment() %>% 
  ACF(.resid) %>%
  autoplot() + 
  guides(color=FALSE) + 
  labs(title="ARIMA ACF for the 50 US States", y="Autocorrelation")+
...
---
### Extracting the Model Details

```{r}
NYT.COVID %>% as_tibble() %>% left_join(., state_ranks %>% select(state, name)  %>% distinct(), by=c("state" = "name")) %>% as_tsibble(index=date, key=state) %>% 
  model(ARIMA = ARIMA(New.Cases)) %>% 
  filter(state=="Oregon") %>% 
  report()
```
