...
First, on knots.  Trends are not always *monotonic*.  Witness the example of population in Afghanistan.
...
Describe the outcome as two separate but combinable forms: a Forecast equation $$\hat{y}_{t+h|t} = S_{t}$$ and a Smoothing Equation $$S_{t} = \alpha y_{t} + (1-\alpha) S_{t-1}$$.
...
```
model(ETS(outcome ~ error("A"/"M") + trend(method=c("N","A","Ad")) + season(method = c("N", "A", "M"))))
```
...
Information criterion [IC] provide a measure of goodness of fit for models that may differ in number of parameters or underlying form.  
