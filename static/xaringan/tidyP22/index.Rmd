...
knitr::opts_chunk$set(fig.retina=3, fig.height=6, fig.width=9)
```
...
```{r}
library(skimr)
FastFood %>% group_by(restaurant) %>% skim(calories) %>% kable() %>% kable_styling() %>% scroll_box(width = "800px", height = "500px")
```

---

```{r}
FastFood %>% group_by(restaurant) %>% skim(protein) %>% kable() %>% kable_styling() %>% scroll_box(width = "800px", height = "500px")
```

---

```{r}
FastFood %>% group_by(restaurant) %>% skim(sodium) %>% kable() %>% kable_styling() %>%
