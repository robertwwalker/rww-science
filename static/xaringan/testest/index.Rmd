...
```{r, echo=FALSE}
library(cowplot)
PI <- ggplot(WH.Means, aes(x=Mean.Expenditure)) + geom_density(color="magenta") + theme_nothing() + labs(x="Mean.Expenditures")
ggdraw(BasePlot) +
  draw_plot(PI, .18, 0.3, .15, .15)
```
...
```{r}
ResQD <- data.frame(ResQ)
BasePlot + geom_vline(data=ResQD, aes(xintercept=ResQ), color="blue")
```
