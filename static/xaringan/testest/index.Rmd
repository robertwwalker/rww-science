...
**Single Mean**  
*The average of some quantitative outcome with appropriate uncertainty.<sup>1</sup>*    
...
**Compare Means**  
*Compare averages/means across two groups with appropriate uncertainty.<sup>1</sup>*   
...
Two key features:
1. Expectation: $n \cdot \pi$ or number of subjects times probability [$\pi | n=1$].
2. Variance is $n \cdot \pi \cdot (1 - \pi)$ [$\pi(1-\pi) | n=1$] and standard deviation is $\sqrt{n \cdot \pi \cdot (1 - \pi)}$ [$\sqrt{\pi(1-\pi)} | n=1$.
...
```{r Yellow, cache=TRUE, echo=FALSE}
...
```{r GGAnim, eval=FALSE, warning=FALSE, message=FALSE, results='hide', gganimate = list(fig.height="600px", fig.width="1200px")}
...
```{r, fig.width=10}
ggplot(WH, aes(x=Ethnicity, y=Expenditures, fill=Ethnicity)) + geom_violin() + theme_xaringan_inverse() + scale_xaringan_fill_discrete()
```
