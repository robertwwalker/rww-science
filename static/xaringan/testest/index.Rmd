...
$$ [\hat{p}_{M} - \hat{p}_{F}] \pm z*\sqrt{ \frac{\hat{p}_{M}(1-\hat{p}_{M})}{n_{M} } + \frac{\hat{p}_{F}(1-\hat{p}_{F})}{n_{F}} } $$
...
If we assume [or recognize it is generally unknownable] that the male and female samples are independent, the covariance is zero, and the variance of the difference is simply the sum of the variance for male and female, respectively.

$$SE(\hat{p}_{M} - \hat{p}_{F}) =  \sqrt{\frac{\hat{p}_{M}(1-\hat{p}_{M})}{n_{M}} + \frac{\hat{p}_{F}(1-\hat{p}_{F})}{n_{F}}} $$
