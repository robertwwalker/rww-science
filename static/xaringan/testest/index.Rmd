...
Results from the combination of a normal random variable in the numerator and a $\chi^2$ random variable in the denominator.
...
The paired t-test is a single sample average applied to the measured difference.  The key is knowing who is matched with whom and believing that there is at least some variation in both outcomes that depends on the unit in question.  Take the example of *Concrete*.
...
For each batch, we can calculate the difference between those with an without an additive.  To do so, we need *mutate*.  Let's plot it.
...
Concrete.Diff %>% ggplot() + aes(x=Sampled.Mean.Difference) + geom_density()
```
