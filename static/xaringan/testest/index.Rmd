...
knitr::opts_chunk$set(warning=FALSE, message=FALSE, fig.showtext = TRUE, fig.retina=3, dev = "svg", fig.height=6)
library(DT)
library(here)
library(svglite)
library(magrittr)
...
*The probability of some qualitative outcome with appropriate uncertainty.*   
*The average of some quantitative outcome with appropriate uncertainty.*    


*Compare key probabilities across two groups.*  
*Compare averages/means across two groups.*  
...
We will first focus on the former.
...
This approximation yields an estimate of $\pi$, with 90% confidence, that ranges between `r prop.test(table(UCBTab$Admit), conf.level = 0.9, correct=FALSE)$conf.int[[1]]` and `r prop.test(table(UCBTab$Admit), conf.level = 0.9, correct=FALSE)$conf.int[[2]]`.
...
UCB.Pi %>% ggplot(., aes(x=Pi.Admit, fill=Gender)) + geom_density(alpha=0.5) + theme_xaringan_inverse() + transition_states(frameN, state_length = 40, transition_length = 20) + enter_fade() + exit_fade() -> SaveAnim
animate(SaveAnim)
...
Why do we focus on the average?  Under general conditions, the average has a known probability distribution even if the distribution of the underlying data is unknown.  This is a result of the central limit theorem and a generic property of averaging.
...
ggsave(BasePlot, filename="BaseDensity.svg", device="svg", path=here("static/xaringan/testest/images"))
```
...
```{r, fig.width=10}
(ggplot(WH, aes(x=Expenditures)) + geom_density() +  theme_xaringan_inverse()) / (ggplot(WH.Means, aes(x=Mean.Expenditure)) + geom_density() + theme_xaringan_inverse())
```
...
```{r, echo=FALSE}
library(cowplot)
PI <- ggplot(WH.Means, aes(x=Mean.Expenditure)) + geom_density(color="magenta") + geom_area(data=ResQD, aes(x=ResQ, y=Height2), fill="blue", color="blue", alpha=0.2, outline.type = "full") + theme_nothing() + labs(x="Mean.Expenditures")
ggdraw(BasePlot) +
  draw_plot(PI, .24, 0.3, .15, .15)
```
