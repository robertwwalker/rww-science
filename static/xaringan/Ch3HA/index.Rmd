...
food %>% autoplot(Turnover) +
  labs(y = "Turnover ($AUD)")  + theme_xaringan()
```
...
```{r food-anim, echo=FALSE, interval=1/10, message=FALSE, fig.height=5, fig.width=8}
food %>%
  ...
  view_follow() +
  ggtitle("Box-CoxT(lambda = {format(1 - as.numeric(closest_state), digits = 2)})")  + theme_xaringan()
```
...
```{r stlwindowanim, echo=FALSE, warning=FALSE, message=FALSE, interval=1/10,  fig.height=5.35, fig.width=8, eval=TRUE}
