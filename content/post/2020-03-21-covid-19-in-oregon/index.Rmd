...
m2 <- leaflet() %>% addProviderTiles("OpenStreetMap.Mapnik") %>% setView(lat=43.8041, lng=-120.5542, zoom=6) %>% addTiles()  %>% addPolygons(data=Map.L, color = "white", fillOpacity = 0.3, fillColor = mypal(Map.L$Number.of.cases), weight=0.5, highlightOptions = highlightOptions(color = "white", weight = 2, bringToFront = TRUE), label = lapply(Map.L$PTT, htmltools::HTML)) %>% addLegend("bottomleft", pal = mypal, values = Map.L$Number.of.cases, title = "Coronavirus Cases in Oregon", opacity = 0.3)
frameWidget(m2)
```
