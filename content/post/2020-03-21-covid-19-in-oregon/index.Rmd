...
The Oregon data are available from OHA but not in an easy form.  The URL that I found is up to date as of 3/21/2020, 8:00 a.m: [here](https://govstatus.egov.com/OR-OHA-COVID-19).  `datapasta` makes it easy to cut and paste them though I will figure out how to automate that shortly.

```{r}
Oregon.COVID <- data.frame(
...
Oregon.COVID <- Oregon.COVID %>% mutate(PTT = paste("Cases:",Number.of.cases,"<br> Deaths:",Deaths,"<br>Negative Tests:",Negative.test.results, sep=""))
...
ggplot(Map.Me, aes(geometry=geometry, fill=Number.of.cases, label=County.x)) + 
  ...
  geom_label_repel(stat = "sf_coordinates",
    ...
    size = 3,
    box.padding = unit(0.05, "lines"))  + scale_fill_continuous_tableau("Red") + theme_minimal() + labs(title="COVID-19 in Oregon", x="", y="", caption="https://govstatus.egov.com/OR-OHA-COVID-19")
```
