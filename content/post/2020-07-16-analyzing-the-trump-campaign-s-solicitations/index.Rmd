...
summary: ''
authors: []
lastmod: '2020-07-16T22:51:38-07:00'
featured: no
image:
  caption: ''
  focal_point: ''
...
Trump.from %>% mutate(email_From = str_remove(email_from, "<contact@victory.donaldtrump.com>")) %>% mutate(email_From = str_remove(email_From, "<contact@campaigns.rnchq.com>")) %>% group_by(email_From) %>% summarise(Count = n()) %>%  ggplot(., aes(x=fct_reorder(email_From, Count), y=Count, fill=email_From)) + geom_col() + coord_flip() + guides(fill=FALSE) + labs(x="From Whom?")
```
