...
COVID.Head <- COVID.Head %>% mutate(Outcome = as.numeric(str_remove_all(Outcome, ",")), date=as.Date(date), Scraped.date = Scraped.date) # %>% filter(!startsWith(.[[1]],"Total"))
COVID.County <- webpage %>%
        ...
        mutate(date=as.Date(date), 
               ...
               Negative.test.results = as.numeric(str_remove_all(Negative.test.results, ",")))
COVID.Age <- webpage %>%
        html_nodes("table") %>%
        .[3] %>%
        html_table(fill = TRUE) %>% data.frame()  %>%  
