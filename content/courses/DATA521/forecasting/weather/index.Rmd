...
```{r AccuseR}
Accuse.Usual.Suspect <- function(train, test, Outcome, DateVar) {
  Outcome <- ensym(Outcome)
  DateVar <- ensym(DateVar)
fits <- train %>%  model(`K = 1` = ARIMA(!!Outcome ~ fourier(K=1)),
      `K = 2` = ARIMA(!!Outcome ~ fourier(K=2)),
      `K = 3` = ARIMA(!!Outcome ~ fourier(K=3)),
      ARIMA = ARIMA(!!Outcome),
      ETS = ETS(!!Outcome),
...
Min.ForeCPlot <- FC %>% filter(.model==Min.Model$.model) %>%
  ...
  autolayer(train %>% slice_max(., order_by=!!DateVar, n=24) %>% select(!!Outcome)) +
  theme_ipsum_rc() + 
