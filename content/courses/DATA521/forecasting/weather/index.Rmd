...
Now I need to find the actual data.
...
I want to write a magic function.  It takes three inputs: train, test, Outcome, and DateVar.  The first two are data; split into train and test.  The final inputs are `Outcome`, the outcome without quotation marks, and the Date variable [the index of the tsibble], also unquoted.

```{r AccuseR}
Accuse.Usual.Suspect <- function(train, test, Outcome, DateVar, H.Horizon=12) {
  # Turn the symbols -- names that will make sense in their environments when called -- that the user supplies into symbolics.
  Outcome <- ensym(Outcome)
...
Min.ForeCPlot <- FC %>% filter(.model==Min.Model$.model) %>%
  ...
  autolayer(train %>% slice_max(., order_by=!!DateVar, n=H.Horizon*2) %>% select(!!Outcome)) +
  theme_ipsum_rc() + 
...
MinAvg.Res <- Accuse.Usual.Suspect(train=NWS.Monthly.Train, test=NWS.Monthly.Test, Outcome=MinAvg, DateVar=Month)
```
...
Precip.Res <- Accuse.Usual.Suspect(train=NWS.Monthly.Train, test=NWS.Monthly.Test, Outcome=Precip, DateVar=Month)
```
