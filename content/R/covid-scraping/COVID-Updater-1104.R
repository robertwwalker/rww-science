rm(list=ls())
library(curl); library(rvest); library(htmltools); library(tidyverse); library(rlang); library(here); library(stringr)
...
#    mutate(COVID.19.Details = str_replace(COVID.19.Details, "Current patients", "COVID-19 patients"))
  return(list(Header=COVID.Head, Counties = COVID.County, Gender = COVID.Gender, Ages = COVID.Age, Hospitalized = COVID.Hospitalized))
}
# Had to remove , Hospital.Cap=COVID.Hospital.Cap, COVID.Strain = COVID.Strain because of failure to report on November 4, 2020.
