...
# Building an Oregon COVID data

I now have a few days of data now.  To rebuild it, I will have to use the [waybackmachine](https://archive.org/web/).  These data are current as of March 24, 2020.  The files that I need to locate follow updates to [this page from Oregon's OHA](https://govstatus.egov.com/OR-OHA-COVID-19).
...
COVID.Head <- COVID.Head %>% 
  mutate(Outcome = comma.rm.to.numeric(Outcome), date=as.Date(date), Scraped.date = Scraped.date)
...
COVID.Head <- COVID.Head %>% mutate(Outcome = as.numeric(str_remove(Outcome,","), date=as.Date(date), Scraped.date = Scraped.date)
# Extract the county data
COVID.County <- webpage %>%
        html_nodes("table") %>%
        .[2] %>%
...
# March 21 Update
Mar21 <- OHA.Corona.2(website="https://web.archive.org/web/20200321211741/https://govstatus.egov.com/OR-OHA-COVID-19", date="2020-03-21")
# March 22 Update
Mar22 <- OHA.Corona.2("https://web.archive.org/web/20200322205228/https://govstatus.egov.com/OR-OHA-COVID-19", date="2020-03-22")
# March 23 Update
...
Tomorrow, it will be time to create the automation for updating the data.
