...
In this post, I want to visualize the data on COVID-19 in the state of Oregon.  I now have a few days of data that I want to scrape, separate, clean up, and plot.  The process of scraping it is detailed in [a separate file](https://rww.science/r/covid-scraping/).  These data are current as of March 25, 2020.
...
```{r}
Test2 <- Oregon.Tests %>% ggplot(.) + aes(x=date, y=Outcome, fill=Category, label=Outcome) + geom_col(position = "fill") + scale_fill_viridis_d() + labs(title="COVID-19 Testing in Oregon", y="Tests", x="Date", subtitle="Total shown as label", caption = "data: https://govstatus.egov.com/OR-OHA-COVID-19")
Test2
```
...
Age2 <- OR.Ages1 %>% ggplot(.) +
 ...
 scale_fill_ipsum() + labs(x="Date", y="Percent of COVID-19 Positives", caption="@PieRatio from OHA data") +
 theme_minimal()
Age1 + Age2
```
...
Age2 <- OR.Ages1 %>% ggplot(.) +
 aes(x = date, fill = Age.group, weight = Number.of.cases) +
 geom_bar(position = "fill") +
 scale_fill_viridis(discrete=TRUE) + labs(x="Date", y="Percent of COVID-19 Positives", caption="@PieRatio from OHA data") +
 theme_minimal()
Age1 + Age2
```
...
Res.1 <- Map.Me %>% filter(date=="2020-03-25") %>%
